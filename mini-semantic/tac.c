...
TAC *declare_var_typed(const char *name, int sym_type)
{
	...
	if (scope)
		sym = lookup_sym(sym_tab_local, (char *)name);
	else
		sym = lookup_sym(sym_tab_global, (char *)name);
...
void out_tac(FILE *f, TAC *i)
{
	...
	case TAC_VAR:
		int basetype = SYM_VAR;
		if (i->a->etc)
			basetype = *((int *)i->a->etc);

		if (basetype == SYM_CHAR)
			fprintf(f, "var %s (char)", to_str(i->a, sa));
		else
			fprintf(f, "var %s (int)", to_str(i->a, sa));
		break;
