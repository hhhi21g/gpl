...
%token <string> INTEGER IDENTIFIER TEXT CHAR_CONST CHAR
...
%type <tac> program function_declaration_list function_declaration function parameter_list statement assignment_statement return_statement if_statement while_statement call_statement block variable_list declaration_list declaration statement_list input_statement output_statement variable_list_char
...
variable_list_char
  ...
  | IDENTIFIER '=' CHAR_CONST
    {
        SYM *v = mk_var(strdup($1));   
    	v->type = SYM_CHAR;
  ...
  | variable_list_char ',' IDENTIFIER
    {
        $$ = join_tac($1, declare_var_typed($3, SYM_CHAR));  /* char a,b; */
    }
  | variable_list_char ',' IDENTIFIER '=' CHAR_CONST
    {
        SYM *v = mk_var($3);
        v->type = SYM_CHAR;
        EXP *rhs = mk_exp(NULL, mk_char($5[1]), NULL);
        $$ = join_tac($1, do_assign(v, rhs));    /* char a,b='A'; */
    }
  ;
